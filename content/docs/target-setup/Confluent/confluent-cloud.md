---
pageTitle: Documentation for Confluent Cloud as target
...
Specify your Confluent Cloud connection details to Replicant with a connection configuration file. You can find a sample connection configuration file `confluent.yaml` in the `$REPLICANT_HOME/conf/conn` directory.
...
In the preceding configuration, `max-connections` specifies the maximum number of connections Replicant can open in Confluent Cloud. Feel free to change its value as you need.
{{< /tab >}}
...
- Enter the [bootstrap hostname and port number](https://docs.confluent.io/platform/current/installation/configuration/producer-configs.html#bootstrap-servers) in the **Bootstrap Host** and **Port** fields.
...
- Specify the maximum number of connections Replicant can open in Confluent Cloud in the **Max connections** field. Defaults to `30`. 
...
If you want to define data mapping from your source to Confluent Cloud, specify the mapping rules in the mapper file. For more information on how to define the mapping rules and run Replicant CLI with the mapper file, see [Mapper Configuration]({{< ref "/docs/references/mapper-reference" >}}).
...
1. From `$REPLICANT_HOME`, naviagte to the sample Confluent Cloud Applier configuration file:
   ```BASH
...
2. The configuration file contains global Applier parameters, with snapshot and realtime parameters following the global parameters:
    ...
    For snapshot mode, the following Confluent Cloud-specific parameters are available:
