...
When data is being loaded into S3, the data is first converted to either a CSV or JSON file. To better understand the data format for the CSV and JSON converted files, see [S3 File Format]({{< ref "/docs/references/s3-file-format" >}} "S3 File Format").

{{< hint "warning" >}} 
**Important:** We highly recommended that you read the [S3 File Format page](/docs/references/s3-file-format) when using S3 as the Target system.
{{< /hint >}}